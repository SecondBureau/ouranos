...
class ApplicationController < ActionController::Base
  ...
  def check_member_confirmation
    ...
    CSV.foreach file.tempfile do |row|
      ...
      family_name = row[0]
      family_name << " "+row[1] if row[1]
			family = Family.find(:first, :conditions => ["lower(name) LIKE ?", family_name.tr(acc,'_').downcase])
			...
			unless ::Rails.env.eql?('production')
			  email_test = "#{row[5].gsub('@romain.2bu.ro','').gsub('@','_')}@romain.2bu.ro"
			end
			...
			if row[0] && row[1]
			  ...
			  father = Person.find(:first, :conditions => ["lower(firstname) LIKE ? AND lower(lastname) LIKE ?", row[1].tr(acc,'_').downcase, row[0].tr(acc,'_').downcase])
          ...
          family.people.build({
			...
			if row[2] && row[3]			
			  mother = Person.find(:first, :conditions => ["lower(firstname) LIKE ? AND lower(lastname) LIKE ?", row[3].tr(acc,'_').downcase, row[2].tr(acc,'_').downcase])
      ...
      j = 12
      while !row[j].nil? do
        kid = Person.find(:first, :conditions => ["lower(firstname) LIKE ? AND lower(lastname) LIKE ?", row[j+1].tr(acc,'_').downcase, row[j].tr(acc,'_').downcase])
        if kid
          people_ids << kid.id
          family.people << kid
        elsif
		    ...
		    if params[:add][family.name]
		      ...
		      family.people.each do |person|
		        ...
		        if params[:add][family.name].index(index.to_s)
		          puts "Person à ajouter"
		          if !family.id
		            puts "Nouvelle famille, donc nouvel user"
		          ...
		          puts "family dans person.family"
		          #person.save!
		          puts "person saved"
